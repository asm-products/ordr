class SessionsController < ApplicationController
  ...
  def failure
    redirect_to login_path, alert: t('session.messages.failed_login.omniauth')
  end

  def destroy
    session[:user] = nil
    redirect_to login_path, notice: t('session.messages.logout')
  end
...
private
  def login_with_omniauth
    ...
    if session[:user]
      ...
      redirect_to jobs_path, notice: t('session.messages.returning_user.omniauth', user: user.name, provider: auth_hash["provider"])
    else
      auth = Authentication.find_or_create(auth_hash)
      session[:user] = auth.user_id.to_s
      redirect_to jobs_path, notice: t('session.messages.new_user.omniauth', user: auth.user.name, provider: auth_hash["provider"])
    end
  end
